...
def training_function(script_args, training_args):
    # Here we are using the tinystories data set 
    ...
    def create_story(sample):
        system_message = (
            "You are a children’s story generator."
            "Users will describe the context (characters, setting, or a plot idea)"
            "and you will write a story based on the provided \nCONTEXT:\n{schema}"
            )
        return {
            "messages": [
                {
    ...
    train_dataset = train_dataset.map(
        create_story, remove_columns=train_dataset.features, batched=False
    )
    eval_dataset = eval_dataset.map(
        create_story, remove_columns=eval_dataset.features, batched=False
    )
